...
namespace SistemaVenta.BLL.Servicios
{
    public class DashBoardService : IDashBoardService
    {
        ...
        {
            DateTime? ultiFecha = tablaVenta.OrderByDescending(v => v.FechaRegistro).Select(v => v.FechaRegistro).First();

            ultiFecha = ultiFecha.Value.AddDays(restarDias);

            return tablaVenta.Where(v=>v.FechaRegistro.Value.Date >= ultiFecha.Value.Date);
        ...
        {
            IQueryable<Producto> _prodQuery = await _productoRepositorio.Consultar();

            int total = _prodQuery.Count();
            return total;
